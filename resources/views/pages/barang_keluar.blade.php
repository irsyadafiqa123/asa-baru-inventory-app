...
@section('header')
    <div class="flex flex-col w-full pb-3 border-b border-gray-300 gap-4 lg:flex-row">
        <h2 class="text-4xl font-semibold text-slate-900 w-full">Barang Keluar</h2>
        <form action="{{ route('barang_keluar.index') }}" method="GET" class="flex gap-2 w-full sm:max-w-80">
...
@section('content')
    ...
    <div x-data="{ openAddItem: {{ session('modal') === 'addStockOutItem' ? 'true' : 'false' }} }" class="relative flex flex-col gap-4 md:flex-row md:flex-wrap w-full">
        ...
        <div x-show="openAddItem" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center" x-transition>
            <div @click.away="openAddItem = false"
                ...
                <div class="flex justify-between items-start mb-4">
                    <h2 class="text-xl font-bold mb-4 w-full text-center">Tambah Barang Keluar</h2>
                    <button @click="openAddItem = false" class=" text-red-600 cursor-pointer">
                        <x-ri-close-fill class="w-8 h-8" />
                    </button>
                </div>

                <form method="POST" action="{{ route('barang_keluar.store') }}" class="flex flex-col gap-6">
                    @csrf
                    ...
                    <div x-data="{
                        ...
                        <input type="text" x-model.debounce.300ms="searchItem" @focus="openItem = true"
                            @input="openItem = true" placeholder="Cari Barang..."
                        ...
                        @error('item_id', 'addStockOutItemError')
                    ...
                    <div>
                        ...
                        @error('amount', 'addStockOutItemError')
        ...
        <div class="space-y-4 w-full mt-2">
                        ...
                        <table class="min-w-full text-left text-sm font-light text-surface">
                            <thead class="border-b border-neutral-300 font-medium">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4 whitespace-nowrap">Harga Jual/Satuan</th>
                                ...
                                @foreach ($stockOuts as $stockOut)
                                    <tr class="font-normal">
                                        <td class="whitespace-nowrap px-6 py-4">
                                            {{ $stockOut->transaction->created_at->format('d/m/Y') }}</td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            {{ $stockOut->item_name }}
                                        </td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            {{ $stockOut->item?->category?->category_name }}
                                        </td>
                                        <td class="whitespace-nowrap px-6 py-4">Rp
                                            {{ number_format($stockOut->selling_price, 0, ',', '.') }}
                                        </td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            {{ $stockOut->amount }}</td>
                                    </tr>
            ...
            {{-- Pagination --}}
            {{ $stockOuts->links('pagination::tailwind') }}
        </div>
    </div>
@endsection
